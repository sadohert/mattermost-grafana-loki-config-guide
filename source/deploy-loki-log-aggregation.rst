...
- A Mattermost **Enterprise license** is active (required for the ``/metrics`` endpoint used by Prometheus; Loki itself is open-source).
...
2. Copy the production configuration file provided in this repository:

   .. code-block:: bash

      sudo cp config/loki-config.yaml /opt/loki/loki-config.yaml
...
2. Copy and edit the Promtail configuration:

   .. code-block:: bash

      sudo cp config/promtail-mattermost.yaml /opt/promtail/promtail-config.yaml

   Edit ``/opt/promtail/promtail-config.yaml`` and replace the placeholders:
   ...
   .. code-block:: bash
      ...
      sudo vi /opt/promtail/promtail-config.yaml
...
A pre-built Grafana dashboard is included in this repository at ``dashboards/mattermost-loki-logs.json``. It provides:
...
2. Select **Upload JSON file** and choose ``dashboards/mattermost-loki-logs.json``.

3. On the import screen, select your **Loki** data source from the dropdown.

4. Select **Import**.

The dashboard uses template variables (``service_name``, ``instance``, ``log_level``, ``search``) that populate automatically from your Loki labels.
...
2. Use the ``config/promtail-postgres.yaml`` configuration file instead of the Mattermost one. Copy it and edit the placeholders:

   .. code-block:: bash

      sudo cp config/promtail-postgres.yaml /opt/promtail/promtail-config.yaml

   - Replace ``<LOKI_HOST>`` with your monitoring server's address.
   - Replace ``<PG_HOSTNAME>`` with this server's hostname.
   - Adjust the ``__path__`` to match your PostgreSQL log location (see comments in the file).
...
.. list-table::
   ...
   * - High disk usage on the monitoring server
     - Review the retention period in ``loki-config.yaml``. Consider reducing it or adding more disk. Check ``du -sh /opt/loki/data/`` to see current usage.
